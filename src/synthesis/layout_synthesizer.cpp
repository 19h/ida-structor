...
LayoutSynthesizer::synthesize_z3(const UnifiedAccessPattern &pattern) {
    ...
    try {
        ...
        if (candidates.empty()) {
            detail::synth_log(
                    "[Structor] No field candidates - falling back to heuristics\n");
        ...
        if (z3_result.is_sat()) {
            ...
            if (result.inferred_packing) {
                result.structure.alignment =
                        std::min(result.structure.alignment, *result.inferred_packing);
            }
            ...
            // Unknown or error
            detail::synth_log("[Structor] Z3 returned unknown/timeout - falling back "
                                                "to heuristics\n");
...
void LayoutSynthesizer::apply_bitfield_recovery(
    ...
    for (const auto &field : result.fields) {
        ...
        for (const auto &bf : bfs) {
            ...
            bf_field.type =
                    field.type.empty() ? make_base_type(field.size) : field.type;
...
void LayoutSynthesizer::detect_subobjects(const UnifiedAccessPattern &pattern,
    ...
    for (sval_t delta : ordered_deltas) {
        ...
        if (!suggested_type_name.empty()) {
            ...
            fallback_type_name.sprnt("auto_%s",
                                                             make_substruct_field_name(delta).c_str());
...
LayoutSynthesizer::synthesize_with_type_inference(cfunc_t *cfunc, int var_idx,
    ...
    if (config_.use_type_inference) {
        ...
        if (infer_result.success) {
            detail::synth_log(
                    "[Structor] Type inference completed: %zu variables typed\n",
...
LayoutSynthesizer::apply_synthesis_result(cfunc_t *cfunc, int var_idx,
    ...
    qstring reason;
    bool applied = applicator.apply_variable(cfunc, var_idx, inferred_ptr,
